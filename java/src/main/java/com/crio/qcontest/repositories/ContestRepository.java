...
public class ContestRepository implements IContestRepository {
    ...
    public Contest save(Contest contest) {
        Contest q = new Contest(contest.getTitle(), contest.getLevel(), contest.getCreator(),
                contest.getQuestions(), autoIncrement);
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        List<Contest> allContest = findAll();

        if(level != null){
            return allContest.stream().filter(ct -> ct.getLevel().equals(level))
